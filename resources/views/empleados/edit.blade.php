...
@section('content')
    <h2 class="text-center mb-5">Actualizar empleado: {{$empleado->name}}</h2>


    <div class="row justify-content-center mt-5">
        <div class="col-md-8">
        <a href="{{ route('empleado.index') }}" class="btn btn-success mb-5">volver al index</a>
            <form method="POST" action="{{ route('empleado.update', ['id' => $empleado->id]) }}" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="from-group">
                    ...
                    <select
                        ...
                        id="empresa_id"
                    >
                        <option value="{{$empleado->empresa->name}}">-- Seleccione --</option>
                        @foreach ($empresas as $empresa)
                            <option 
                                value="{{ $empresa->id }}" 
                                {{ $empleado->empresa->name == $empresa->name ? 'selected' : '' }} 
                            >{{$empresa->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text"
                        ...
                        id="phone"
                        placeholder="Sitio web de la empleado"
